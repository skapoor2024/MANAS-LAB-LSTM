...
def mel_fbank_mx(winlen_nfft, fs, NUMCHANS=20, LOFREQ=0.0, HIFREQ=None):
    ...
    for i in xrange(NUMCHANS):
        ...
        mfb[cind[i+1]:cind[i+2],
            i] = (cbin_mel[i+2]-fbin_mel[cind[i+1]:cind[i+2]]) / (cbin_mel[i+2]-cbin_mel[i+1])
...
def read_lab_to_bool_vec(lab_file, true_label=None, length=0, frame_rate=100.):
    ...
    labels = np.atleast_2d(np.loadtxt(
        lab_file, usecols=(0, 1, 2), dtype=object))
    if true_label:
        labels = labels[labels[:, 2] == true_label]
    start, end = np.rint(frame_rate/1e7*labels.T[:2].astype(int)).astype(int)
    if not end.size:
        return np.zeros(min_len, dtype=bool)
    frms = np.repeat(np.r_[np.tile([False, True], len(end)), False], np.r_[
                     np.c_[start - np.r_[0, end[:-1]], end-start].flat, max(0, min_len-end[-1])])
