...
if os.path.isfile(fea_output):
    ...
    if len(signal)==0: logging.info("Could not read file %s, no features will be created", audio_input)
        ...
        #print(fea.shape)
        #print(fea)
        fea= np.r_[ np.repeat(fea[[0]], left_ctx, axis=0), fea, np.repeat(fea[[-1]], right_ctx, axis=0)] #got this
        #print(fea)
        #print(fea.shape)
        nn_input=nn_def.preprocess_nn_input(fea,left_ctx_bn1,right_ctx_bn1)
        #######$$$$$$$$$$$$$$$$ Use this section if we want to save MFEC features
        #print("nn input shape")
        #print(nn_input.shape)
        #nn_input.dump(fea_output) # 144D NN input/MFEC features extracted by this pgm 
        #print(fea_output)
        ########$$$$$$$$$$$$$$$ till here
